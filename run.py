...
When running locally, you can use any of the following approaches:
    ...
    - You can run your Flask app directly from the command line. However,
        you'll need to specify the app to run. For example, to run `journeyapp`
        you'll need to enter `python -m flask --app journeyapp run`. This method
        doesn't use run.py.
...
in your WSGI configuration file:
    ...
    - Import the `app` object directly from the `journeyapp` module. For example,
        in PythonAnywhere's WSGI file, set the final line to `from journeyapp
